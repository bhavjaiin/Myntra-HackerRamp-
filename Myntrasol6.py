...
fashion_data.to_csv('fashion_data_processed.csv', index=False)
print("Fashion DataFrame:\n", fashion_data.head())  # Debug statement

# Step 2: Model Training

data = pd.read_csv('fashion_data_processed.csv')
X = data['Trend']
...
model.save('fashion_ai_model.h5')

# Step 3: Interactive Platform

app = Flask(__name__)
model = tf.keras.models.load_model('fashion_ai_model.h5')
...
def generate_tutorial():
    ...
    tutorial = create_tutorial(predicted_label)
...
def update_model():
    ...
    new_fashion_data.to_csv('new_fashion_data.csv', index=False)

    new_data = pd.read_csv('new_fashion_data.csv')
